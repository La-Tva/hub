  ...
  return (
    <div className="flex h-full text-white/90">
      ...
      <div className="flex-1 flex flex-col">
        ...
        <div className="h-12 border-b border-white/5 flex items-center justify-between px-4">
          ...
          <div className="relative">
            ...
            <input 
              ...
              className={cn(
                "bg-white/5 border border-white/5 rounded-md pl-9 pr-3 py-1 text-xs outline-none focus:border-white/20 transition-all",
                isMobile ? "w-32" : "w-48"
              )}
            />
          </div>
        ...
        {/* Grid View */}
        <div className={cn(
          "p-6 grid gap-6 overflow-y-auto",
          isMobile ? "grid-cols-2" : "grid-cols-4"
        )}>
          {files.map((file) => (
            <div key={file.name} className="flex flex-col items-center gap-2 group cursor-pointer">
              <div className={cn(
                "bg-white/5 rounded-xl flex items-center justify-center border border-white/5 group-hover:bg-white/10 group-hover:border-white/20 transition-all shadow-lg",
                isMobile ? "w-20 h-20" : "w-16 h-16"
              )}>
                <File className={isMobile ? "w-10 h-10 text-blue-400/60" : "w-8 h-8 text-blue-400/60"} />
              </div>
